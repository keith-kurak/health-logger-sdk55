...
export const STATS: StatConfig[] = [
  { name: 'water',         label: 'Water',          unit: 'servings', inputType: 'integer',       aggregate: 'sum',    color: '#3B82F6', icon: { ios: 'drop.fill',               android: 'water_drop',     web: 'water_drop'    } },
  { name: 'fruit',         label: 'Fruit',          unit: 'servings', inputType: 'integer',       aggregate: 'sum',    color: '#F59E0B', icon: { ios: 'apple.logo',              android: 'nutrition',      web: 'nutrition'     } },
  { name: 'veggies',       label: 'Veggies',        unit: 'servings', inputType: 'integer',       aggregate: 'sum',    color: '#22C55E', icon: { ios: 'leaf.fill',               android: 'eco',            web: 'eco'           } },
  { name: 'cardio',        label: 'Cardio',         unit: 'min',      inputType: 'integer',       aggregate: 'sum',    color: '#EF4444', icon: { ios: 'figure.run',              android: 'directions_run', web: 'directions_run' } },
  { name: 'steps',         label: 'Steps',          unit: 'steps',    inputType: 'integer',       aggregate: 'sum',    color: '#A855F7', icon: { ios: 'figure.walk',             android: 'directions_walk', web: 'directions_walk' } },
  { name: 'weight',        label: 'Weight',         unit: 'lbs',      inputType: 'decimal',       aggregate: 'latest', color: '#64748B', decimalPlaces: 1, icon: { ios: 'scalemass.fill',      android: 'scale',          web: 'scale'         } },
  { name: 'bloodPressure', label: 'Blood Pressure', unit: 'mmHg',     inputType: 'bloodPressure', aggregate: 'latest', color: '#EC4899', icon: { ios: 'heart.text.square.fill', android: 'monitor_heart',  web: 'monitor_heart' } },
  { name: 'sleep',         label: 'Sleep',          unit: 'hrs',      inputType: 'decimal',       aggregate: 'sum',    color: '#6366F1', decimalPlaces: 1, icon: { ios: 'moon.fill',           android: 'bedtime',        web: 'bedtime'       } },
];
...
export function addEntry(name: StatName, value: string, dateKey: string, ts = Date.now()): void {
...
export function getEntriesForDay(entries: Entry[], dateKey: string): Entry[] {
  return entries.filter((e) => e.dateKey === dateKey).sort((a, b) => a.ts - b.ts);
}

/** Latest raw value for a day, or null. */
export function getLatestForDay(entries: Entry[], dateKey: string): string | null {
...
export function formatStatValue(config: StatConfig, raw: string | null): string {
  if (raw === null) return '—';
  if (config.inputType === 'bloodPressure') {
    try {
      const { systolic, diastolic } = JSON.parse(raw) as { systolic: number; diastolic: number };
...
export function getDayChartValue(config: StatConfig, entries: Entry[], dateKey: string): number | null {
  const dayEntries = getEntriesForDay(entries, dateKey);
  if (dayEntries.length === 0) return null;
  if (config.aggregate === 'sum') {
    return dayEntries.reduce((sum, e) => sum + (numericValue(config, e.value) ?? 0), 0);
  }
...
export function getDayDisplayValue(config: StatConfig, entries: Entry[], dateKey: string): string {
  const dayEntries = getEntriesForDay(entries, dateKey);
  if (dayEntries.length === 0) return '—';
  if (config.aggregate === 'sum') {
    const total = dayEntries.reduce((sum, e) => sum + (numericValue(config, e.value) ?? 0), 0);
    return config.decimalPlaces !== undefined ? total.toFixed(config.decimalPlaces) : String(total);
  }
...
function seedDay(date: Date): void {
  ...
  if (rand(0, 1)) addEntry('veggies', String(rand(1, 3)), dk, ts(18, rand(0, 30)));
  ...
  addEntry('weight', (171 + rand(0, 4) + rand(0, 9) / 10).toFixed(1), dk, ts(7, rand(0, 15)));
...
/** Seeds today's data on every app load if no entries exist for today yet. */
export function seedTodayIfNeeded(): void {
  const todayKey = localDateKey(new Date());
  const hasData = STATS.some((s) => getEntriesForDay(readEntries(s.name), todayKey).length > 0);
  if (hasData) return;
  seedDay(new Date());
}
